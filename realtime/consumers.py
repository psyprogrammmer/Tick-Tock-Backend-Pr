...
class SecondUserConsumer(AsyncJsonWebsocketConsumer):
    async def connect(self):
        ...
        # await self.channel_layer.group_add(self.kwargs.get('grp_name'), self.channel_name)
        data = await sync_to_async(requests.post)("http://localhost:8000/secondclient/",
    ...
    async def disconnect(self, close_code):
        ...
        if close_code != 3001:
            await sync_to_async(requests.post)("http://localhost:8000/groupdelete/", {"grp_name": self.kwargs["grp_name"]})
